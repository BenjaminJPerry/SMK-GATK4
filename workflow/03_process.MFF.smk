...
rule filter_monomorphic_bcftools: 
    ...
    shell:
        ...
        zcat {input.norm} | grep -P -v "0\/1\S+\t0\/1\S+" | grep -P -v "1\/1\S+\t1\/1\S+" |  bcftools view - -O z8 -o {output.filtered};
...
rule filter_MQ60_bcftools:
    ...
    output:
        ...
        csi = "results/05_private/{samples}.MFF.norm.monomorphic.DPFilt.eva.bcftools.intersection.MQ60.vcf.gz.csi",
    threads: 8
    conda:
        "bcftools-1.19"
    resources:
